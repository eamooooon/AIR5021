...
import actionlib
from sagittarius_vlm_agent.msg import SGRCtrlAction, SGRCtrlGoal, SGRCtrlResult, SGRCtrlFeedback
...
class MoveItSGRTool:
    def __init__(self, init_pose=True, end_effector=None):
        ...
        # 设置允许的最大速度和加速度
        self.max_acceleration_scaling = float(rospy.get_param("~max_acceleration_scaling", 0.8))
        self.max_velocity_scaling = float(rospy.get_param("~max_velocity_scaling", 0.8))
        self.execute_wait = float(rospy.get_param("~execute_wait", 0.08))
    ...
    def to_pose_eular(self, times, px, py, pz, roll=0, pitch=0, yaw=0):
        ...
        if len(plan.joint_trajectory.points) != 0:
            ...
            rospy.sleep(min(times, self.execute_wait))
            return True
        else:
